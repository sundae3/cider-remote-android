...
//            var hasStickyHeaderShown = false  // To control sticky header visibility
            ...
            items(nonReorderableItems, key = { it.id }) { item ->
                ...
                var fixedArtworkUrl = item.attributes.artwork.url.replace("{w}x{h}", "128x128")
                    Surface(shadowElevation = 0.dp, color = Color.Transparent, modifier = Modifier.padding(top=4.dp)) {
                        Row(
                            modifier = Modifier
                                .fillMaxWidth()
                            ...
                            Row(verticalAlignment = Alignment.CenterVertically) {
                                ...
                                Column(modifier = Modifier.padding()) {
                                    Text(
                                        text = mainViewModel.trimText(item.attributes.name, characterLimit),
                                    ...
                                    Text(
                                        text = mainViewModel.trimText(item.attributes.name, characterLimit),
                                        Modifier.padding(horizontal = 8.dp, vertical = relativeSizeInDpHeight),
                        ...
                        Box(modifier = Modifier.fillMaxSize()) {
                            ...
                            SwipeToDismissBox(
                                ...
                                backgroundContent = {
                                    ...
                                    val isBeingDismissed = swipeState.currentValue == SwipeToDismissBoxValue.EndToStart ||
                                        ...
                                        Row(
                                            ...
                                            Row(
                                                ...
                                                Column(modifier = Modifier.padding()) {
                                                    Text(
                                                        text = mainViewModel.trimText(item.attributes.name, characterLimit),
                                                        modifier = Modifier.padding(start = 8.dp, end=8.dp, bottom = relativeSizeInDpHeight),
                                                        color = Color.White,
                                                        fontSize = 16.sp,
                                                    )
                                                    Text(
